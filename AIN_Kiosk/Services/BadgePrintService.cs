...
namespace AIN_Kiosk.Services
{
    ...
    // Service responsible for handling Zebra badge print operations via ZPL spools.
     //Fully purged of hardcoded real-person identities and static company names.


    public class BadgePrintService
    {
        private const string ZebraPrinterName = "ZDesigner GK420t";
        private readonly PrinterAdapter _printerAdapter = new();

        // Prints visitor badge using active workflow parameters and dedicated badge token.
        
        public async Task<bool> PrintVisitorBadgeAsync(
            string hostName,
            string purpose,
            string mobile,
            string badgeToken,
            ...
            string companyName = "")
        {
            // Dynamic resolution: use workflow input or fallback to generic synthetic labels
            string displayVisitorName = string.IsNullOrWhiteSpace(visitorName) ? "Guest Visitor" : visitorName;
            string displayCompany = string.IsNullOrWhiteSpace(companyName) ? "Visitor" : companyName;
...
^FO55,138^A0N,30,30^FR^FD{displayVisitorName}^FS
^FO45,190^GB450,50,50,B^FS
^FO55,202^A0N,26,26^FR^FD{displayCompany}^FS
...
^FO540,110^BQN,2,6^FDQA,{badgeToken}^FS
^FO530,395^A0N,24,24^FDID:^FS
^FO575,385^GB170,42,2,B^FS
^FO590,395^A0N,20,20^FD{badgeToken}^FS

^FO16,616^GB768,568,3,B^FS
^FO45,655^A0N,55,55^FDVISITOR^FS
^FO45,725^GB450,55,55,B^FS
^FO55,738^A0N,30,30^FR^FD{displayVisitorName}^FS
^FO45,790^GB450,50,50,B^FS
^FO55,802^A0N,26,26^FR^FD{displayCompany}^FS
...
^FO540,710^BQN,2,6^FDQA,{badgeToken}^FS
^FO530,995^A0N,24,24^FDID:^FS
^FO575,985^GB170,42,2,B^FS
^FO590,995^A0N,20,20^FD{badgeToken}^FS
^XZ";
